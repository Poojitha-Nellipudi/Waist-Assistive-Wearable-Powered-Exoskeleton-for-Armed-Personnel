...
th1 = -40*pi/180; th2 = 45*pi/180;
th3 = -160*pi/180;
th4 = -10 * pi/180; th5 = -10*pi/180;
...
mu_a = [0;0.0;psi]+[r*(1-cos(t(j)));-r*(t(j)-sin(t(j)));0];
if j>1
    q =q_r(:,j-1);
else
...
mu_a = [-xx;-yy;psi]+[r*(1-cos(t(j)));-r*(t(j)-sin(t(j)));0];
if j>1
    q =q_r(:,j-1);
else
    q = [th1;th2;th3;th4;th5] ;
...
biped_c = R* [x1,x2,x3,x4,x5,x;
              y1,y2,y3,y4,y5,y];
plot([0,0.08,biped_c(1,1),0]+0.4,[0,0,biped_c(2,1),0],'r-','linewidth',3)
hold on
plot([biped_c(1,6),biped_c(1,6)+0.08,biped_c(1,5),biped_c(1,6)]+0.4,[biped_c(2,6),biped_c(2,6),biped_c(2,5),biped_c(2,6)],'r-','linewidth',3)
plot(biped_c(1,1:5)+0.4,biped_c(2,1:5),'r-o','linewidth',3,'MarkerEdgeColor','k',...
                       'MarkerFaceColor','g',...
                       'MarkerSize',10)
plot(prof_bp(1,:)+0.4,prof_bp(2,:),'k-')
axis([-0.2 0.8 -0.1 0.9])
axis square
set(gca,'fontsize',16)
xlabel('x,[m]');
